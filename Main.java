...
public class Main {
	...
	static String maiorNota(double[] notas) {
		String maiorNota = "";
		double analisarMaior = 0.0;

		for (int i = 0; i < notas.length; i++) {
			if (notas[i] > analisarMaior) {
				analisarMaior = notas[i];
	...
	static void AvIntegrada(double[] notas) {
		double AI = 0.0;
		AI = lerNota("Digite nota da AI", NOTA_MAX_AVALIACOES[0]);
